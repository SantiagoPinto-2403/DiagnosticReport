document.getElementById('diagnosticReportForm').addEventListener('submit', function(event) {
    ...
    // Obtener solo la fecha (sin hora) de los campos datetime-local
    const effectiveDate = document.getElementById('effectiveDateTime').value.split('T')[0];
    const issuedDate = document.getElementById('issued').value.split('T')[0];
    ...
    // Crear objeto DiagnosticReport con solo fechas (sin hora)
    const diagnosticReport = {
        ...
        effectiveDateTime: effectiveDate, // Solo fecha
        issued: issuedDate, // Solo fecha
        performer: [{
